...
func New() (*DNSServer, error) {
	...
	return NewDNSServer(*addres, *resolver)
}
...
func (s *DNSServer) createAnswers(questions []message.DNSQuestion) ([]message.DNSAnswer, error) {
	...
	fallbackAnswers := make([]message.DNSAnswer, 0, len(questions))
...
func (s *DNSServer) forwardRequest(questions []message.DNSQuestion) []message.DNSAnswer {
	...
	request := message.GenerateDNSQuery(0, questions)
	...
	response, err := message.NewDNSResponse(buf[:size])
	if err != nil {
		return nil
	}
...
func (s *DNSServer) createAnswerForQuestion(question message.DNSQuestion) (*message.DNSAnswer, error) {
	return message.NewDNSAnswer(
...
func (s *DNSServer) processNextRequest(buf []byte) error {
	...
	request, err := message.NewDNSRequest(buf[:size])
	if err != nil {
